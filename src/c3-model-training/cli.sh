...
export DISPLAY_NAME="fixer_training_job_$UUID"
export MACHINE_TYPE="n1-standard-4"
export REPLICA_COUNT=1
export EXECUTOR_IMAGE_URI="us-docker.pkg.dev/vertex-ai/training/tf-gpu.2-12.py310:latest"
export PYTHON_PACKAGE_URI=$GCS_BUCKET_URI/fixer-app-trainer.tar.gz
...
export CMDARGS="--model_name=vgg16,--epochs=5,--batch_size=32,--wandb_key=$WANDB_KEY"
#export CMDARGS="--model_name=vgg16,--epochs=3,--batch_size=64,--wandb_key=$WANDB_KEY"
...
gcloud ai custom-jobs create --region=$GCP_REGION --display-name=$DISPLAY_NAME --python-package-uris=$PYTHON_PACKAGE_URI --worker-pool-spec=machine-type=$MACHINE_TYPE,replica-count=$REPLICA_COUNT,executor-image-uri=$EXECUTOR_IMAGE_URI,python-module=$PYTHON_MODULE --args=$CMDARGS