...
DATA_COLLECTOR_IMAGE = "us-central1-docker.pkg.dev/ac215-399020/vertex-ai-pipeline/c1-data-scraping:latest"
DATA_PROCESSOR_IMAGE = "us-central1-docker.pkg.dev/ac215-399020/vertex-ai-pipeline/c2c-data-preprocessing:latest"
...
def main(args=None):
    ...
    if args.data_collector:
        # Define a Container Component- image scraping
        ...
        # Define a Pipeline for image scraping
        @dsl.pipeline
        def data_collector_pipeline():
            data_collector()

        # Build yaml file for image scraping pipeline
        compiler.Compiler().compile(
            data_collector_pipeline, package_path="image_scraping.yaml"
        )
        ...
        DISPLAY_NAME = "rehab-ai-image-scraping-" + job_id
        job = aip.PipelineJob(
            display_name=DISPLAY_NAME,
            template_path="image_scraping.yaml",
    ...
    if args.data_processor:
        ...
        job_id = generate_uuid()
        DISPLAY_NAME = "rehab-ai-data-processor-" + job_id
        job = aip.PipelineJob(
            display_name=DISPLAY_NAME,
    ...
    if args.model_training:
        ...
        job_id = generate_uuid()
        DISPLAY_NAME = "rehab-ai-model-training-" + job_id
        job = aip.PipelineJob(
            display_name=DISPLAY_NAME,
    ...
    if args.model_deploy:
        ...
        job_id = generate_uuid()
        DISPLAY_NAME = "rehab-ai-model-deploy-" + job_id
        job = aip.PipelineJob(
            display_name=DISPLAY_NAME,
